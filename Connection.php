...
class Connection extends DatabaseConnection {
  ...
  public function lastInsertId($name = NULL): string { return $this->getMasterConnection()->lastInsertId($name); }
  ...
  public function prepareQuery($query, $quote_identifiers = true) { return $this->getSlaveConnection()->prepareQuery($query); }
  ...
  function updateInvalidationFile($key, $target, $idx, $status) {
    ...
    if ($databases) {
      ...
      $output = '<' . '?php' . "\n";
      if (!is_numeric($idx)) {
        watchdog('autoslave', "[$key][$target][$idx] is not a valid connection!", array(), RfcLogLevel::ERROR);
        return;
      }
  ...
  function determineMasterTarget() {
    if (!$this->__master) {
      $this->__master = self::$autoslave_id;
    }  
